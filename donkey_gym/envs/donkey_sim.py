...
class DonkeyUnitySimHandler(IMesgHandler):
    def __init__(self, conf: Dict[str, Any]):
        ...
        # self.image_array = np.zeros((54, 160, 3))
    ...
    def on_need_car_config(self, message: Dict[str, Any]) -> None:
        logger.debug("on need car config")
        self.loaded = True
        logger.debug("car config:")
    ...
    def send_config(self, conf: Dict[str, Any]) -> None:
        ...
        logger.debug("sending car config.")
        ...
        if "car_config" in conf.keys():
            ...
            logger.debug("done sending car config.")
        ...
        #     )
        if True:
            cam_config = {"img_w": 160,
                            "img_h": 54,
                            "img_d": 3
                          }
            # logger.info("cam config:")
            # logger.info(cam_config)
            self.send_cam_config(**cam_config)
            logger.debug(f"done sending cam config. {cam_config}")
    ...
    def on_recv_message(self, message: Dict[str, Any]) -> None:
        # logger.debug("on_recv_message", message)
    ...
    def reset(self) -> None:
        ...
        # r = ROI
        # print("r", r)
        # image = self.image_array[int(r[1]):int(r[1] + r[3]), int(r[0]):int(r[0] + r[2])]
        ...
        self.image_array_b = None
        self.last_obs = self.image_array
        logger.info("self.image_array.shape:"+str(self.image_array.shape))
        # logger.info("self.camera_img_size:" + str(self.camera_img_size))
        self.time_received = time.time()
        self.last_received = self.time_received
        self.hit = "none"
        self.cte = 0.0
    ...
    def calc_reward_orig(self, done: bool) -> float:
    ...
    def calc_reward(self, done: bool) -> float:
        ...
        # Collision
        if done or self.hit != "none":
            print("REWARD -1000")
            return -1000

        # if self.cte > self.max_cte:
        print("max cte:{} cte:{:.2f} REWARD:{:.2f}".format(self.max_cte, self.cte, math.pow(2, self.max_cte-abs(self.cte))))
        return math.pow(2, self.max_cte-abs(self.cte))
    ...
    def determine_episode_over(self):
        ...
        if math.fabs(self.cte) > 2 * self.max_cte:
            ...
            logger.info(f"game over: cte {self.cte}")
            self.over = True
        elif self.hit != "none":
            logger.info(f"game over: hit {self.hit}")
            self.over = True
        elif self.missed_checkpoint:
            logger.info("missed checkpoint")
            self.over = True
        elif self.dq:
    ...
    def on_recv_scene_names(self, message: Dict[str, Any]) -> None:
        if message:
            ...
            logger.debug("loading scene", self.SceneToLoad)
    ...
    def send_control(self, steer: float, throttle: float, brake: float = 0.0) -> None:
        ...
        # logger.debug("CONTROL: {steer:"+str(round(steer, 4))+", throttle:"+str(round(throttle, 4))+", brake:"+str(round(brake, 4))+"}")
